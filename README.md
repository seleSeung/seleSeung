<!-- 헤더 타이핑 배너 -->
<p align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1200&color=8AB4F8&center=true&vCenter=true&width=600&lines=Hi%2C+I'm+Seungyeon+%F0%9F%91%8B;Java+%26+MySQL+lover;Learning+Cloud+%2F+Infra;Welcome+to+my+GitHub!"/>
  </a>
</p>

<!-- 간단 소개 -->
<h3 align="center">👩‍💻 Seungyeon Lee | Java · MySQL · Cloud Learner</h3>

<p align="center">
  문제를 구조화하고 객체지향적으로 풀어내는 걸 좋아합니다.<br/>
  자바로 기초를 단단히 쌓고, MySQL·클라우드 운영을 학습하며 작은 서비스들을 만들어가고 있어요.
</p>

<!-- 뱃지 -->
<p align="center">
  <a href="mailto:selee@example.com">
    <img alt="Email" src="https://img.shields.io/badge/Contact-Email-informational?style=flat&logo=gmail&logoColor=white&color=0A84FF"/>
  </a>
  <a href="https://www.linkedin.com/in/">
    <img alt="LinkedIn" src="https://img.shields.io/badge/LinkedIn-profile-blue?style=flat&logo=linkedin"/>
  </a>
  <img alt="Profile Views" src="https://komarev.com/ghpvc/?username=seleSeung&style=flat&color=grey"/>
</p>

---

## ✨ About me
- 🔭 최근: 자바로 **객체지향 연습(도서관/배달 등 도메인 모델링)**, MySQL로 **스키마 설계 & 쿼리 최적화** 실습
- ☁️ 관심사: **AWS 기초 인프라(EC2, VPC, ALB, 보안그룹)**, 배포 자동화
- 🧠 학습 스타일: 작은 단위로 쪼개서 실습 → 리팩터링 → 문서화

---

## 🧰 Tech Stack
<!-- 필요한 것만 남기고 조정하세요 -->
<p align="left">
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring-6DB33F?style=for-the-badge&logo=spring&logoColor=white"/>
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white"/>
  <img src="https://img.shields.io/badge/JPA%2FHibernate-59666C?style=for-the-badge&logo=hibernate&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white"/>
  <img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws&logoColor=white"/>
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white"/>
</p>

---

## 📦 Projects (Pinned)
<!-- repo 이름만 바꿔서 고정 카드로 보여줄 수 있어요 -->
<p align="left">
  <a href="https://github.com/seleSeung/one-meal">
    <img height="120" src="https://github-readme-stats.vercel.app/api/pin/?username=seleSeung&repo=one-meal&theme=tokyonight&show_owner=true"/>
  </a>
  <a href="https://github.com/seleSeung/library-oop">
    <img height="120" src="https://github-readme-stats.vercel.app/api/pin/?username=seleSeung&repo=library-oop&theme=tokyonight&show_owner=true"/>
  </a>
</p>

> 💡 **설명 예시**  
> - `one-meal`: 배달 플랫폼 MVP (계층형 아키텍처, MySQL 스키마 설계, AWS 3-Tier 실습)  
> - `library-oop`: Book/Member/Library 객체 모델링, 테스트 주도 리팩터링

---

## 📈 Stats
<p>
  <img height="165" src="https://github-readme-stats.vercel.app/api?username=seleSeung&show_icons=true&theme=tokyonight&rank_icon=github"/>
  <img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=seleSeung&layout=compact&langs_count=8&theme=tokyonight"/>
</p>

<p>
  <img height="165" src="https://streak-stats.demolab.com?user=seleSeung&theme=tokyonight&hide_border=false"/>
</p>

---

## 🗂️ Study Notes
- 객체지향 연습 문제 정리: _도메인 이벤트, 불변 객체, 의존성 역전_  
- MySQL: **정규화 → 인덱스 → 실행 계획(EXPLAIN)** 으로 병목 파악, N+1/풀스캔 방지  
- 클라우드: **VPC 서브넷/라우팅 → 보안그룹 → 로드밸런서** 흐름으로 설계 메모

---

## 📫 Contact
- 이메일: `lomeme0114@naver.com` (원하시는 메일로 바꿔주세요)
- 이슈/PR 환영합니다! 피드백으로 더 단단한 코드를 만듭니다 😊

---

<!-- 풋터 -->
<p align="center">
  Made with ❤️ & Focus — keeping things simple, readable, and testable.
</p>
